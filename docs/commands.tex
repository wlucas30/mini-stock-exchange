...
\begin{itemize}
    ...
    \item graph a participant's cash balance and net worth over time;
...
show-command =
    ...
    | "SHOW", "PERFORMANCE", participant-id
    | "SHOW", "TRADES"
    | "SHOW", "TIME"
    | "SHOW", "GRAPH", symbol
    | "SHOW", "STATS", symbol
...
\subsection{\texttt{SHOW PERFORMANCE}}

\subsubsection*{Syntax}

\begin{lstlisting}
SHOW PERFORMANCE <participant_id>
\end{lstlisting}

\subsubsection*{Description}

Displays the participant's total cash balance and calculated net worth as two
lines against simulation time. The simulation records both values at time zero
and after every subsequent simulation step. Total cash includes funds reserved
for active buy orders. Net worth adds positions marked at the exchange's public
reference prices, using acquisition cost when no market reference is available.
...
The command fails if the simulation is unavailable or no performance history
exists for the specified participant.
...
per step. While the command is running, the terminal displays an in-place
progress bar with the completed and total step counts.
...
Displays observed market statistics for the specified instrument over the
last 1,000 simulation steps. Statistics include the latest trade and midpoint,
the current bid-ask spread, trade count, traded volume, volume-weighted average
price, and realised midpoint volatility. Volatility is the sample standard
deviation of consecutive midpoint log returns and is not annualised.
