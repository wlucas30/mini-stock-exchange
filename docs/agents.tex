...
When an instrument is first created, its initial fundamental value is the
configured starting price. Its complete initial supply is allocated directly
to configured participants at that acquisition cost. No startup orders are
created automatically. The instrument begins in a neutral sentiment regime.
Its initial volatility is 0.1\%.
...
Volatility is represented as a proportion of fundamental value over 1,000
simulation steps rather than as a fixed number of ticks or a per-step rate. For
example, volatility of 0.001 means approximately 0.1\% volatility over 1,000
steps. The per-step volatility is obtained by dividing this value by the square
root of 1,000.

Volatility receives a small random shock on every simulation step, but is pulled
gradually back towards its normal level and remains within bounded limits.
Sentiment supplies a target growth rate which adjusts gradually as sentiment
changes and returns towards neutral when sentiment is neutral. Fundamental
value is maintained internally with floating-point precision and does not
mean-revert to its initial price. It is rounded to integer ticks only when
exposed to the rest of the simulation.
...
Momentum traders make decisions from observable market behaviour rather than
fundamental information. For every instrument, the implemented strategy records
the public reference price on each simulation step and maintains 20-step and
100-step rolling averages. Once both windows are full, it compares the short
average with the long average.

A momentum trader may buy when the 20-step average is at least 0.5\% above the
100-step average, or sell when it is at least 0.5\% below. It uses a marketable
limit order at the current best ask or bid and only trades when its available
cash or position permits.
...
one-sided. Hard inventory bounds also limit quoted quantities so a fully filled
bid or ask cannot move the maker outside its permitted inventory range.

\subsection{Long-term holders}

Long-term holders represent investors who own shares without providing regular
order flow. They retain their configured positions and do not submit or cancel
orders.
...
& \textbf{Momentum trader} & \textbf{Market maker} \\
\hline
...
Automated strategies may assign an optional expiry timestamp to a limit order.
The exchange cancels due orders as simulation time advances and releases any
remaining reserved cash or positions.
